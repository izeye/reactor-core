...
=== Using the `composeDefer` Operator

The `composeDefer` operator is similar to `composeNow` and also lets you encapsulate operators
...
The following image shows how the `composeDefer` operator works with per-subscriber transformations:
